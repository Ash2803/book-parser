...
from parse import get_page, parse_book_page, check_for_redirect, RedirectedPage, parse_collection_page, \
    get_collection_last_page_number
...
def main():
    ...
    books = []
    collection_last_page = get_collection_last_page_number(get_page('https://tululu.org/l55/'))
    for page_number in range(args.start_page, args.end_page):
        try:
            ...
            for book_num, book_link in enumerate(books_links, 1):
                try:
                    ...
                    if not args.skip_imgs:
                        book_image_path = download_book_image(
                            book_image_link,
                            book_title,
                            book_num,
                            args.dest_folder
            ...
            logging.exception('URL had been redirected')
            continue
        if page_number == collection_last_page:
